...
//!
//! Kept free of dependencies so `agent-companion-hook` can use it without pulling in the
//! server half of this crate.

/// Legacy pipe identity, retained so old hooks and new apps share one server.
...
pub const PIPE_NAME_ENV: &str = "AGENT_COMPANION_PIPE_NAME";

/// Set to `1`/`true`/`yes` to make the hook exit immediately without connecting.
pub const SKIP_HOOKS_ENV: &str = "AGENT_COMPANION_SKIP_HOOKS";

/// The configured pipe name, from [`PIPE_NAME_ENV`] or the default.
pub fn pipe_name() -> String {
    match crate::compat::var(PIPE_NAME_ENV) {
...
/// `AGENT_COMPANION_PIPE_NAME` accepts either spelling.
...
/// The full path Agent Companion listens on and the hook connects to.
...
pub fn hooks_disabled() -> bool {
    match crate::compat::var(SKIP_HOOKS_ENV) {
        Ok(value) => matches!(
...
mod tests {
    ...
    fn bare_names_are_expanded() {
        assert_eq!(pipe_path("agent-companion"), r"\\.\pipe\agent-companion");
        assert_eq!(
            pipe_path("agent-companion-test-7"),
            r"\\.\pipe\agent-companion-test-7"
        );
    }
