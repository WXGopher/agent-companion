...
use super::{EntryStatus, files::commit_files};

const MARKER: &str = "Managed by Agent Companion";
const RECORD: &str = "atoll-install.json";
const FEATURE_COMMENT: &str = " # Managed by Agent Companion";
...
fn managed(entry: &Value) -> bool {
    entry
        ...
        .is_some_and(|marker| marker == MARKER || marker == "Managed by Atoll")
}
...
pub fn install_codex(home: &Path, binary: &Path) -> io::Result<CodexReport> {
    if !binary.is_file() {
        return Err(io::Error::new(
            ...
            "agent-companion-hook binary not found",
        ));
    }
...
mod tests {
    ...
    fn scratch() -> (tempfile::TempDir, PathBuf) {
        ...
        let hook = dir.path().join("agent-companion-hook.exe");
    ...
    #[test]
    fn legacy_install_reuses_its_original_restore_record() {
        let (dir, new_hook) = scratch();
        let home = dir.path();
        let legacy_hook = home.join("atoll-hook.exe");
        fs::write(&legacy_hook, "legacy binary").unwrap();
        fs::write(
            home.join("config.toml"),
            "[features]\nhooks = false # user choice\n",
        )
        .unwrap();
        install_codex(home, &legacy_hook).unwrap();
        // Recreate an installation written by Atoll, including its TOML marker.
        for name in ["config.toml", "hooks.json", RECORD] {
            let path = home.join(name);
            let text = fs::read_to_string(&path)
                .unwrap()
                .replace("Managed by Agent Companion", "Managed by Atoll");
            fs::write(path, text).unwrap();
        }
        let record = fs::read(home.join(RECORD)).unwrap();
        assert!(
            status_codex(home)
                .unwrap()
                .entries
        ...
        assert!(install_codex(home, &new_hook).unwrap().changed);
        assert_eq!(fs::read(home.join(RECORD)).unwrap(), record);
        assert!(!install_codex(home, &new_hook).unwrap().changed);
        let hooks = read_hooks(home).unwrap();
        ...
        assert!(
            fs::read_to_string(home.join("config.toml"))
                .unwrap()
                .contains("hooks = false # user choice")
        );
        assert_eq!(fs::read_to_string(legacy_hook).unwrap(), "legacy binary");
    }
}
