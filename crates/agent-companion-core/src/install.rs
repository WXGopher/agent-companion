//! Installation and removal of Agent Companion's hook wiring in Claude Code and Codex configuration files.
//!
//! # Rules this module plays by
//!
//! `settings.json` belongs to the user, not to Agent Companion. So:
//!
//! - The file is manipulated as a [`Value`] tree. There is no settings struct,
//!   because a struct would silently drop every key Agent Companion does not know about.
//! - Entries Agent Companion owns are recognized by [`MANAGED_MARKER`] appearing in their
...
//!
//! Agent Companion writes the exec form **for hooks**. As a safety net for any build that
//! ignores `args` and runs the bare `command`, `command` is the plain absolute
//! path and `agent-companion-hook` defaults `--source` to `claude` — so the degraded path
...
//! `args` key at all. An entry written in the exec form there runs `agent-companion.exe`
...
//! `agent-companion.exe` and its `agent-companion-hook.exe` into `%LOCALAPPDATA%\Agent Companionin` first,
//! and the install points at those.
//!
//! # The usage bridge, and why it is no longer installed
//!
//! Agent Companion used to install itself as Claude Code's status line, because the
...
//! and Agent Companion no longer needs the slot at all.
//!
...
//! puts Agent Companion on the critical path of something the user looks at constantly,
...
//! under [`ORIGINAL_STATUSLINE_KEY`], carries every key of theirs that Agent Companion
//! does not own onto the wrapper (`refreshInterval` and anything else Claude
//! Code reads off that object), and `agent-companion statusline` runs the stashed command
//! with Agent Companion's own stdout, so its bytes and exit code reach Claude Code
...
/// Substring that marks a hook entry as Agent Companion's. Present in every `command`
/// Agent Companion writes, because the command is a path ending in `agent-companion-hook.exe`.
pub const MANAGED_MARKER: &str = "agent-companion-hook";

fn managed_hook(command: &str) -> bool {
    let command = command.to_ascii_lowercase();
    command.contains(MANAGED_MARKER) || command.contains("atoll-hook")
}
...
/// The `settings.json` key holding the status line entry Agent Companion displaced.
///
...
// Retain the legacy restore key so an upgrade never loses the user's original.
...
/// The subcommand Agent Companion's status line entry invokes.
pub const STATUSLINE_SUBCOMMAND: &str = "statusline";

/// One hook entry Agent Companion installs into Claude Code.
...
/// The events Agent Companion subscribes to in Claude Code.
///
...
/// Who owns `settings.json`'s `statusLine`, and whether Agent Companion is wrapping
/// something.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BridgeState {
    /// No status line at all.
    Absent,
    /// Agent Companion's status line, with nothing behind it.
    Installed,
    /// Agent Companion's status line, delegating to the user's saved original.
    Wrapping,
    /// Someone else's status line. Agent Companion never removes one of these.
    Foreign,
}
...
impl BridgeState {
    ...
    /// Whether Agent Companion's bridge is what Claude Code will run.
    pub fn is_agent_companion(self) -> bool {
...
///
/// The default is [`BridgePolicy::Skip`]. Agent Companion reads Claude Code's usage from
...
pub enum BridgePolicy<'a> {
    ...
    #[default]
    Skip,
    /// Install Agent Companion's status line into an empty slot, and refresh it if it is
    /// already ours. An entry belonging to anyone else is left alone.
    IfEmpty(&'a Path),
    /// Install over an existing entry, stashing it so `agent-companion statusline` can run
...
/// The absolute path of the running `agent-companion` executable, which is what the status
/// line entry has to name — Claude Code runs it with an arbitrary working
/// directory, so a relative path would not resolve.
pub fn agent_companion_binary_path() -> io::Result<PathBuf> {
    std::env::current_exe()
}

/// Where Agent Companion keeps the copies of itself that `settings.json` points at:
...
/// Copy the running `agent-companion.exe` and its sibling `agent-companion-hook.exe` into
...
pub fn install_binaries() -> io::Result<StableBinaries> {
    ...
    let running = agent_companion_binary_path()?;
...
/// Add (or refresh) Agent Companion's hooks in `settings_path`, and treat `statusLine`
/// according to `bridge`.
///
...
/// Remove every hook entry carrying [`MANAGED_MARKER`] and Agent Companion's status line,
...
pub fn uninstall_claude(settings_path: &Path) -> io::Result<InstallReport> {
    ...
    {
        strip_managed(hooks);
        // An empty `hooks` object left behind is Agent Companion's litter — install is
...
pub fn statusline_state(settings: &Value) -> BridgeState {
    match settings.get(STATUSLINE_KEY) {
        ...
        Some(entry) if is_agent_companion_statusline(entry) => {
...
/// The status line entry Agent Companion displaced, for `agent-companion statusline` to delegate to.
...
/// Whether a `statusLine` entry is one Agent Companion wrote.
///
/// Recognized by the pair "names the agent-companion binary" and "invokes the statusline
/// subcommand", so a user command that merely lives in a directory called
/// `agent-companion` is not mistaken for ours.
pub fn is_agent_companion_statusline(entry: &Value) -> bool {
    let command = entry
        .get("command")
        .and_then(Value::as_str)
        .unwrap_or_default()
        .to_ascii_lowercase();
    if (command.contains("agent-companion") || command.contains("atoll"))
        && command.contains(STATUSLINE_SUBCOMMAND)
    {
        return true;
    }

    // Recognise the exec form an older Agent Companion wrote, so that an install can
    ...
    (command.contains("agent-companion") || command.contains("atoll"))
        && entry
            .get("args")
...
/// The keys Agent Companion owns in a `statusLine` entry. Everything else in the entry
/// belongs to the user and is copied through verbatim.
const STATUSLINE_OWNED_KEYS: [&str; 3] = ["args", "command", "type"];

/// The `command` string for Agent Companion's status line: the binary and the subcommand,
/// as one shell string.
///
...
/// and nothing else, so an entry carrying `args` runs `agent-companion.exe` with no
...
pub fn statusline_command(agent_companion_binary: &Path) -> String {
    let path = agent_companion_binary.to_string_lossy();
    if path.contains(' ') {
...
/// `siblings`, when given, is the entry Agent Companion is replacing. Every key in it that
/// Agent Companion does not own is carried over — `refreshInterval` above all, which
/// Claude Code reads off this same object and which a wrapper that dropped it
/// would silently reset to the default.
fn statusline_entry(agent_companion_binary: &Path, siblings: Option<&Map<String, Value>>) -> Value {
    ...
    entry.insert(
        "command".to_string(),
        Value::String(statusline_command(agent_companion_binary)),
    );
...
fn install_statusline(root: &mut Map<String, Value>, policy: BridgePolicy<'_>) -> bool {
    let agent_companion_binary = match policy {
    ...
    let ours = current.as_ref().is_some_and(is_agent_companion_statusline);
    ...
    root.insert(
        ...
        statusline_entry(agent_companion_binary, siblings),
    );
    false
}

/// Take Agent Companion's status line back out, restoring what it displaced.
fn uninstall_statusline(root: &mut Map<String, Value>) {
    let ours = root
        .get(STATUSLINE_KEY)
        .is_some_and(is_agent_companion_statusline);
            ...
            let command = hooks
                ...
                .and_then(|groups| {
                    groups
                        ...
                        .find(|command| managed_hook(command))
                        .map(str::to_string)
                });
            EntryStatus {
                event: spec.event.to_string(),
...
/// Drop every managed hook from every event, including events Agent Companion no longer
...
/// — or a whole event key — that Agent Companion emptied goes with its entries, because
/// Agent Companion is what put it there. One that was *already* empty when we arrived is
...
/// Append Agent Companion's entry to the matching group under `spec.event`, creating the
...
mod tests {
    ...
    fn fake_hook_binary() -> PathBuf {
        PathBuf::from(r"C:\Tools\AgentCompanion\bin\agent-companion-hook.exe")
    }
    ...
    /// on an event Agent Companion also wants.
    ...
    fn install_preserves_unknown_keys_and_user_hooks() {
        ...
        // Agent Companion joined the existing "*" group instead of adding a second one.
    ...
    fn reinstall_replaces_a_stale_binary_path() {
        ...
        install_claude(&path, &fake_hook_binary(), BridgePolicy::Skip).unwrap();
        let moved = PathBuf::from(r"D:\Elsewhere\agent-companion-hook.exe");
    ...
    /// A synthetic `agent-companion.exe` path, for the same reason as the hook one.
    fn fake_agent_companion_binary() -> PathBuf {
        PathBuf::from(r"C:\Tools\AgentCompanion\bin\agent-companion.exe")
    }
    ...
    /// The hooks half of "only add, never change": Agent Companion appends its entries to
    ...
    fn an_install_and_uninstall_round_trip_leaves_the_users_hooks_byte_identical() {
        ...
        let mine = serde_json::json!({
            "hooks": {
                // A group of the user's on an event Agent Companion also wants.
                "PreToolUse": [
                    {"matcher": "*", "hooks": [{"type": "command", "command": "their-tool"}]},
                    // A group they left empty. Not ours to tidy away.
                    {"matcher": "Bash", "hooks": []},
                ],
                // An event Agent Companion does not touch at all.
        ...
        install_claude(
            &path,
            &fake_hook_binary(),
            BridgePolicy::IfEmpty(&fake_agent_companion_binary()),
        )
        .unwrap();
        ...
        // The one normalization Agent Companion does make, and the only difference the
    ...
    fn the_status_line_command_quotes_only_when_it_must() {
        ...
        let plain =
            PathBuf::from(r"C:\Users\me\AppData\Local\AgentCompanion\bin\agent-companion.exe");
        assert_eq!(
            statusline_command(&plain),
            r"C:\Users\me\AppData\Local\AgentCompanion\bin\agent-companion.exe statusline"
        );

        let spaced = PathBuf::from(r"C:\Program Files\AgentCompanion\agent-companion.exe");
        let quoted = statusline_command(&spaced);
        assert!(quoted.starts_with('"'), "got {quoted}");
        assert!(
            quoted.ends_with(r#"agent-companion.exe" statusline"#),
            "got {quoted}"
        );
    }

    /// An entry an older Agent Companion wrote in the exec form is still recognised as
    ...
    fn the_broken_exec_form_is_still_recognised_as_ours() {
        let legacy = serde_json::json!({
            "type": "command",
            "command": r"C:\tools\agent-companion.exe",
            "args": ["statusline"],
        });
        assert!(is_agent_companion_statusline(&legacy));
        assert!(is_agent_companion_statusline(&statusline_entry(
            &fake_agent_companion_binary(),
            None
        )));

        // Somebody else's line is still somebody else's, and a tool that merely
        // has "agent-companion" in its name is not ours either.
        assert!(!is_agent_companion_statusline(&user_statusline()));
        assert!(!is_agent_companion_statusline(&serde_json::json!({
            "type": "command",
            "command": "agent-companion-adjacent-tool",
        })));
    }
    ...
    fn an_install_only_ever_names_the_stable_directory() {
        ...
        let stable = dir.path().join("AgentCompanion").join("bin");
        let agent_companion = stable.join("agent-companion.exe");
        let hook = stable.join("agent-companion-hook.exe");

        install_claude(&path, &hook, BridgePolicy::IfEmpty(&agent_companion)).unwrap();

        let settings = read(&path);
        let rendered = settings.to_string();
        assert!(
            rendered.contains(&agent_companion.to_string_lossy().replace('\\', "\\\\")),
    ...
    fn the_bridge_is_installed_when_there_is_no_status_line() {
        ...
        // entry carrying `args` runs agent-companion.exe with no subcommand and prints
        // nothing — which is what emptied a user's status bar.
        assert_eq!(
            settings["statusLine"],
            serde_json::json!({
                "command": format!("{} statusline", fake_agent_companion_binary().to_string_lossy()),
                "type": "command",
            })
        );
        assert!(
    ...
    fn an_existing_status_line_is_stashed_and_wrapped() {
        ...
        let report = install_claude(
            &path,
            &fake_hook_binary(),
            BridgePolicy::Wrap(&fake_agent_companion_binary()),
        )
        .unwrap();
        ...
        let settings = read(&path);
        assert!(is_agent_companion_statusline(&settings["statusLine"]));
        assert_eq!(settings[ORIGINAL_STATUSLINE_KEY], user_statusline());
        assert_eq!(
            original_statusline(&settings),
            Some(&user_statusline()),
            "the wrap target is what agent-companion statusline will delegate to"
        );

        // Every key of the user's that Agent Companion does not own rides onto the
    ...
    /// line the user already had is not Agent Companion's to replace, wrapper or not.
    #[test]
    ...
    fn refreshing_a_wrapper_keeps_the_users_sibling_keys() {
        ...
        let moved = PathBuf::from(r"D:\Elsewhere\agent-companion.exe");
    ...
    fn the_three_bridge_states_are_told_apart() {
        ...
        assert_eq!(
            statusline_state(&serde_json::json!({
                "statusLine": statusline_entry(&fake_agent_companion_binary(), None),
            })),
            BridgeState::Installed
        );
        assert_eq!(
    ...
    fn only_our_own_status_line_is_recognized_as_ours() {
        ...
        assert!(is_agent_companion_statusline(&serde_json::json!({
            "type": "command",
            "command": r"C:\Tools\AgentCompanion\bin\agent-companion.exe statusline",
        })));
        // Named agent-companion but not our subcommand: someone else's tool.
        assert!(!is_agent_companion_statusline(&serde_json::json!({
            "type": "command",
            "command": r"C:\Tools\AgentCompanion\bin\other.exe",
        })));
        // A status line that merely lives in a directory called agent-companion.
        assert!(!is_agent_companion_statusline(&serde_json::json!({
            "type": "command",
            "command": r"C:\agent-companion\my-line.ps1",
            "args": ["--pretty"],
        })));
        assert!(!is_agent_companion_statusline(&user_statusline()));
    }
    ...
    #[test]
    fn legacy_hooks_upgrade_once_and_restore_the_original_status_line() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("settings.json");
        let original = user_statusline();
        fs::write(
            &path,
            serde_json::to_vec(&serde_json::json!({
                "statusLine": {"type":"command", "command":"C:/Atoll/atoll.exe statusline"},
                "_atollOriginalStatusLine": original,
                "hooks": {"Stop":[{"hooks":[
                    {"type":"command","command":"C:/Atoll/atoll-hook.exe"},
                    {"type":"command","command":"my-hook.exe"}
                ]}]}
            }))
            .unwrap(),
        )
        ...
        assert!(
            status_claude(&path)
                .unwrap()
                .iter()
                .any(|e| e.event == "Stop" && e.installed)
        );
        let hook = fake_hook_binary();
        let app = fake_agent_companion_binary();
        install_claude(&path, &hook, BridgePolicy::Wrap(&app)).unwrap();
        assert!(
            !install_claude(&path, &hook, BridgePolicy::Wrap(&app))
                .unwrap()
                .changed
        );
        let saved = read(&path);
        assert_eq!(saved[ORIGINAL_STATUSLINE_KEY], original);
        assert!(!saved.to_string().contains("atoll-hook.exe"));
        assert_eq!(
        ...
        uninstall_claude(&path).unwrap();
        let saved = read(&path);
        assert_eq!(saved[STATUSLINE_KEY], original);
        assert_eq!(
