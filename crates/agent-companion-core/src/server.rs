//! The named-pipe server the Agent Companion app runs: accepts hook connections, hands
...
/// fail fast on "another Agent Companion is already running") before entering the loop.
...
impl PipeServer {
    ...
    /// owns the name — `first_pipe_instance` makes a second Agent Companion a loud error
    ...
    /// Bind the pipe named by `AGENT_COMPANION_PIPE_NAME`, or the default.
