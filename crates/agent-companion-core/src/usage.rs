...
//! nowhere else. So Agent Companion's status line bridge ([`crate::install`] installs it,
//! `agent-companion statusline` runs it) copies that object into a small cache file,
...
/// older Agent Companion, which is why every read of them is optional.
...
/// everything Agent Companion knows about it between hook events.
...
/// and Claude Code itself — so Agent Companion does not ask more often than this, and
...
/// absence sends Agent Companion to the network. How soon a *failed* ask is retried is
...
/// `%LOCALAPPDATA%\AgentCompanion\claude-usage.json`.
///
...
pub fn claude_usage_cache_path() -> io::Result<PathBuf> {
    Ok(agent_companion_data_dir()?.join("claude-usage.json"))
}
...
/// reading Agent Companion gets for free.
...
///
/// Accepts both the bare response (`{"limits": [...]}`) and Agent Companion's wrapper
...
/// `%LOCALAPPDATA%\AgentCompanion\rl.json`.
pub fn rl_cache_path() -> io::Result<PathBuf> {
    Ok(agent_companion_data_dir()?.join("rl.json"))
}

/// `%LOCALAPPDATA%\AgentCompanion`, falling back to `<home>\.agent-companion` on a machine where
...
pub fn agent_companion_data_dir() -> io::Result<PathBuf> {
    if let Some(local) = std::env::var_os("LOCALAPPDATA").filter(|value| !value.is_empty()) {
        ...
        let current = base.join("AgentCompanion");
        crate::compat::migrate_files(
            &base.join("Atoll"),
            &current,
            &["claude-usage.json", "rl.json"],
        )?;
        return Ok(current);
    }
    ...
    let current = home.join(".agent-companion");
    crate::compat::migrate_files(
        &home.join(".atoll"),
        &current,
        &["claude-usage.json", "rl.json"],
...
mod tests {
    ...
    /// written exactly that way, and Agent Companion read it as empty — showing "usage
    ...
    /// A cache written by an Agent Companion that only knew about rate limits.
    #[test]
