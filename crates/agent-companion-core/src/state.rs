//! What Agent Companion believes is happening in every live agent session.
//!
    ...
    pub fn apply(&mut self, payload: &HookPayload, now: u64) {
        ...
        match event {
            ...
            events::POST_TOOL_USE => {
                ...
                // (Agent Companion started mid-session, or the user answered in the
...
/// Every session Agent Companion is currently tracking, keyed by `session_id`.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn an_out_of_order_post_tool_use_is_ignored() {
        // PostToolUse for a tool we never saw approved — Agent Companion started
