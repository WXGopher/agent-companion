...
//! the payload on to the status line Agent Companion displaced.
//!
...
use agent_companion_core::{install, usage};
...
pub fn run() -> io::Result<()> {
    ...
    // and nowhere else Agent Companion can reach. The model name and context percentage
    // ride along for free, and Agent Companion shows those too.
    ...
    // line. Agent Companion never renders over it — not even when it produces nothing, and
    // not even when it fails to start. An empty line is a bug the user can see
    // and fix; Agent Companion's line quietly standing in their place is one they cannot.
...
/// Run the status line Agent Companion displaced, on Agent Companion's own stdout, and return the
...
/// assumption, no size limit, and no place for Agent Companion to accidentally alter a
/// byte. A status line that emits UTF-16, ANSI colour, or nothing at all reaches
/// Claude Code exactly as it would have without Agent Companion in the way.
///
/// A child that cannot be started, or that dies on a signal, yields `0` and an
/// empty line. Never Agent Companion's own rendering: standing in for the user's status
