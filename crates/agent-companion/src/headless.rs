...
//! changes. `agent-companion` with no subcommand runs the app instead; this is what you
...
use agent_companion_core::server::{ConnectionHandle, Handler, PipeServer};
use agent_companion_core::state::{SessionTable, TableCounts};
...
pub struct Args {
    ...
    /// machine has its permission prompts answered by Agent Companion — including sessions
...
pub fn run(args: &Args) -> io::Result<()> {
    ...
    // Only the serving modes need a runtime, so `agent-companion setup` stays instant.
    ...
    runtime.block_on(async move {
        ...
        // "one Agent Companion at a time" can be tested without a display. See
        // [`crate::single`].
        ...
        outln!("agent-companion: listening on {}", server.path());
        if bound.replaced {
            outln!("agent-companion: an Agent Companion was already running; it stood down");
        }
        if modes.auto_allow {
            outln!(
                "agent-companion: DANGER: approvals are auto-allowed for ALL sessions \
                 (PreToolUse={}, PermissionRequest=object)",
                modes.pre_tool_use.as_str(),
            );
        } else {
            outln!(
                "agent-companion: observing only; PreToolUse={}, PermissionRequest is left \
            ...
            io::Error::new(
                ...
                format!(
                    "cannot listen on {}: the name is already in use \
                     (is Agent Companion already running?) [{error}]",
                    agent_companion_core::pipe::configured_pipe_path()
                ),
            )
        } else {
            error
    ...
    fn on_envelope(&self, envelope: Envelope, connection: ConnectionHandle) {
        match envelope {
            ...
            Envelope::Event { event } => {
                ...
                // A newer Agent Companion is starting and wants this pipe; there is only
                ...
                outln!("[{}] stood down for a new Agent Companion", timestamp());
                std::process::exit(0);
            }
...
impl EventPrinter {
    ...
    fn reply_for(&self, event_name: &str) -> Option<Reply> {
        let reason = || Some("auto-allowed by Agent Companion (headless)".to_string());
