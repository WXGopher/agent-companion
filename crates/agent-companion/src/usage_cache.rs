...
//! readout in Agent Companion is built from.
//!
//! Claude Code's numbers reach Agent Companion through the status line bridge's cache and
...
use agent_companion_core::usage::{self, ClaudeLimits, CodexUsage, WindowUsage};
...
/// `percent`, Codex's rollout calls it `used_percent` — and Agent Companion shows the
...
///
/// The rate limit is on the shared token, not on Agent Companion: retrying quickly
...
/// already paid for is a request Agent Companion never has to make. Most of the time
/// Agent Companion piggybacks and sends nothing at all.
...
/// The freshest Claude reading already on disk: Agent Companion's own cache or the
...
mod tests {
    use super::*;
    use agent_companion_core::usage::UsageLimit;
    ...
    fn a_failure_is_retried_on_its_own_schedule() {
        use agent_companion_core::usage::CLAUDE_USAGE_TTL_SECS;
    ...
    /// and Agent Companion shows how much is **left**. 8 % used is 92 % left.
    #[test]
