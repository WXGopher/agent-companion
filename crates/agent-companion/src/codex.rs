...
use agent_companion_core::questions::{Answers, InputRequest};
...
pub fn run(args: &Args) -> io::Result<()> {
    ...
    crate::out::errln!(
        "Agent Companion: launching Codex with local question cards (experimental app-server transport)."
    );
    let mut terminal = agent_companion_core::protocol::TerminalMeta::default();
    for key in agent_companion_core::protocol::TERMINAL_ENV_VARS {
...
async fn launch(
    ...
    exe: PathBuf,
    cwd: PathBuf,
    terminal: agent_companion_core::protocol::TerminalMeta,
) -> io::Result<()> {
    let job = job::Job::new()?;
    ...
    tui.current_dir(&cwd)
        ...
        .arg("AGENT_COMPANION_CODEX_RELAY_TOKEN")
        .env("AGENT_COMPANION_CODEX_RELAY_TOKEN", &token)
        .kill_on_drop(true);
    ...
    let serve = async {
        ...
        let result = relay(
            ...
            token,
            terminal,
            agent_companion_core::pipe::pipe_path(&agent_companion_core::pipe::pipe_name()),
        )
        .await;
        ...
        tokio::select! {
            line = lines.next_line() => {
                ...
                if let Some(thread) = thread {
                    ...
                    let _ = notify_agent_companion(payload, &pipe_path).await;
                }
                if method == "item/tool/requestUserInput" {
                    ...
                    let task = tokio::spawn(async move {
                        if let Ok(Some(answers)) = ask_agent_companion(&request_copy, &key, &cwd_copy, terminal_copy, &pipe_copy).await {
...
async fn notify_agent_companion(payload: HookPayload, path: &str) -> io::Result<()> {
    ...
    tokio::time::timeout(
        ...
        pipe.write_all(
            agent_companion_core::protocol::encode_line(&envelope)
                .map_err(io::Error::other)?
                .as_bytes(),
        ),
    )
...
async fn ask_agent_companion(
    ...
    let mut payload = HookPayload {
        ...
        cwd: Some(cwd.into()),
        tool_name: Some(agent_companion_core::state::ASK_USER_QUESTION.into()),
...
mod tests {
    ...
    async fn actual_pipe_replies_cross_relay_once_and_terminal_answer_cancels_card() {
        use agent_companion_core::server::PipeServer;
        use tokio_tungstenite::{WebSocketStream, tungstenite::protocol::Role};
        let name = format!("agent-companion-relay-test-{}", std::process::id());
        ...
        // inherit the old response tombstone or the old Agent Companion correlation key.
        backend_write
            .write_all(format!("{request}\n").as_bytes())
            .await
            .unwrap();
