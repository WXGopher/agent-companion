//! Turning a hook payload into the card Agent Companion shows, and a card plus a
...
use agent_companion_core::protocol::{HookDecision, HookPayload, HookSource, events};
use agent_companion_core::state::{ASK_USER_QUESTION, correlation_key};
...
pub struct Card {
    ...
    /// [`agent_companion_core::state::correlation_key`].
    pub key: String,
...
impl Card {
    ...
    pub fn answer(&self, option: usize) -> Option<HookDecision> {
        ...
        HookDecision::allow_for_with_input(
            &self.event,
            Some(format!("answered in Agent Companion: {label}")),
...
mod tests {
    use super::*;
    use agent_companion_core::protocol::events;
    use serde_json::json;
    ...
    fn approval() -> HookPayload {
        payload(json!({
            ...
            "session_id": "s-1",
            "cwd": r"C:\synthetic\agent-companion",
            "tool_name": "Bash",
    ...
    fn a_tool_approval_becomes_an_approval_card() {
        ...
        assert_eq!(card.title, "agent-companion");
    ...
    fn a_pre_tool_use_never_raises_a_card() {
        ...
        // PermissionRequest that follows, which Agent Companion can hold open.
