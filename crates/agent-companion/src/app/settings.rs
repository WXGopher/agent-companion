...
use agent_companion_core::install::{self, BridgePolicy, CLAUDE_HOOKS};
...
impl HookStatus {
    /// Whether Agent Companion is installed at all, which is what the button's label
    /// switches on.
    pub fn is_installed(&self) -> bool {
        self.installed > 0
    }

    /// Whether every hook Agent Companion wants is present. A partial install is still
...
/// Install Agent Companion's hooks, returning the sentence to show underneath.
///
...
/// Remove Agent Companion's hooks, leaving the user's own alone.
pub fn uninstall(settings_path: &Path) -> io::Result<String> {
    let report = install::uninstall_claude(settings_path)?;
    Ok(if report.changed {
        "Removed Agent Companion's hooks. Your own hooks were left alone.".to_string()
    } else {
        "No Agent Companion hooks were installed; nothing was written.".to_string()
    })
}
...
mod tests {
    ...
    fn installing_and_removing_move_the_summary() {
        ...
        let agent_companion = dir.path().join("agent-companion.exe");
        std::fs::write(&hook, b"").unwrap();

        install::install_claude(
            &settings,
            &hook,
            BridgePolicy::IfEmpty(agent_companion.as_path()),
        )
        .unwrap();
        let status = read_status(&settings).unwrap();
        ...
        assert!(
            uninstall(&settings)
                .unwrap()
                .starts_with("No Agent Companion hooks")
        );
    }
    ...
    fn a_partial_install_is_reported_as_partial() {
        ...
        // Rip one event back out behind Agent Companion's back, the way a user editing
