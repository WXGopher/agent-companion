...
/// Whether Windows will start Agent Companion at login — asked of the registry itself,
...
// ------------------------------------------------------------- the taskbar
//
// Agent Companion's usage readout lives *inside* the taskbar rather than beside it: it is
...
/// The notification area inside it — the clock, the tray icons. Agent Companion's readout
...
/// The taskbar, if this shell has one Agent Companion recognises.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Taskbar {
    pub handle: isize,
...
/// reset Agent Companion shows: they are all within a week, and a transition inside that
...
///
/// `agent-companion` is a console binary so that `agent-companion setup` and `agent-companion headless` can
...
pub fn activate_terminal_from(
    ancestors: &[agent_companion_core::protocol::ProcessRef],
    tab_hint: Option<&str>,
) -> bool {
...
pub fn terminal_is_foreground(ancestors: &[agent_companion_core::protocol::ProcessRef]) -> bool {
...
///
/// This runs from a click on Agent Companion's own focused flyout, so this process
...
mod tests {
    ...
    fn the_glyph_prefix_and_case_do_not_hide_a_tab() {
        let tabs = names(&["✳ Say hi", "pwsh in agent-companion", "✶ Fix the panel"]);
