...
//! # Cards
//!
//! The card is the only window Agent Companion opens on its own initiative, and it exists
...
pub fn run() -> io::Result<()> {
    // `agent-companion` is a console binary so `agent-companion setup` and `agent-companion headless` can
    ...
    // Bind the pipe before anything else: it is Agent Companion's identity, and starting
    // takes it from whatever Agent Companion already has it — see [`crate::single`].
    let bridge = bridge::start()?;
    if bridge.replaced {
        errln!("agent-companion: an Agent Companion was already running; it stood down");
    }
    errln!("agent-companion: listening on {}", bridge.path);
    win::detach_console();
...
impl App {
    ...
    fn reply(&self, card: &Card, decision: agent_companion_core::protocol::HookDecision) {
        ...
        if let Err(error) = handle.send(&Envelope::Response {
            ...
            errln!("agent-companion: the hook was no longer listening: {error}");
        }
    }
    ...
    fn apply_run_at_login(&self, enabled: bool) {
        ...
        let exe = match agent_companion_core::install::stable_bin_dir() {
            Ok(dir) if dir.join("agent-companion.exe").exists() => {
                Ok(dir.join("agent-companion.exe"))
            }
        ...
        match outcome {
            Ok(()) => self.note_settings(if enabled {
                "Agent Companion starts at login, running the installed copy."
            } else {
                "Agent Companion no longer starts at login."
            }),
            Err(error) => {
    ...
    fn migrate_login_launch(&self) {
        ...
        let exe = match agent_companion_core::install::stable_bin_dir() {
            Ok(dir) if dir.join("agent-companion.exe").exists() => dir.join("agent-companion.exe"),
...
fn describe_session(state: &agent_companion_core::state::SessionState) -> String {
...
mod tests {
    ...
    fn a_session_is_labelled_by_its_project_and_its_last_word() {
        ...
        assert_eq!(
            session_title(Some("agent-companion"), None, "abc12345"),
            "agent-companion"
        );
        ...
        assert_eq!(
            session_title(Some("agent-companion"), Some("***"), "abc"),
            "agent-companion"
        );
