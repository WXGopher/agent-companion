//! What Agent Companion remembers between runs: where the user last dragged a card to,
...
/// Overrides [`config_dir`] outright. Set it to run Agent Companion against a throwaway
/// config — a test build, say — without disturbing the one the user's own Agent Companion
/// keeps.
pub const CONFIG_DIR_ENV: &str = "AGENT_COMPANION_CONFIG_DIR";

/// `%APPDATA%\AgentCompanion`, falling back to `%LOCALAPPDATA%\AgentCompanion` on the machines
/// where the roaming variable is not set, and to [`CONFIG_DIR_ENV`] ahead of
/// both when it is set.
pub fn config_dir() -> io::Result<PathBuf> {
    if let Some(explicit) =
        agent_companion_core::compat::var_os(CONFIG_DIR_ENV).filter(|value| !value.is_empty())
    {
    ...
    agent_companion_core::compat::migrate_files(
        &base.join("Atoll"),
        &current,
        &["config.json", "display.json"],
    )?;
    Ok(current)
}
...
pub struct Config {
    ...
    /// a config written by any other version of Agent Companion — older or newer —
...
pub struct TaskbarConfig {
    ...
    /// cheapest place Agent Companion has to put a number somebody checks all day.
    pub enabled: bool,
...
mod tests {
    ...
    #[test]
    fn keys_from_a_newer_agent_companion_survive_a_rewrite() {
        let config: Config =
    ...
    fn the_taskbar_readout_defaults_to_on() {
        ...
        // A config an older Agent Companion wrote may have no taskbar section at all,
