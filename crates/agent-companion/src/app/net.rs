...
//! # Why WinHTTP
//!
//! Agent Companion needs exactly one request: Claude Code's usage endpoint, at most once
...
//! store the rest of Windows trusts, and needs no dependency Agent Companion does not
//! already have.
//!
...
pub fn get_json(host: &str, path: &str, headers: &[(&str, &str)]) -> io::Result<String> {
    let agent = wide("Agent Companion");
...
mod tests {
    ...
    /// distinguishes "Agent Companion cannot reach the endpoint" from "the endpoint said
    ...
    fn the_fallback_chain_produces_a_reading() {
        use agent_companion_core::usage;
        ...
        let limits = crate::usage_cache::fetch_claude_limits(
            ...
            agent_companion_core::usage::CLAUDE_USAGE_TTL_SECS,
        );
        println!(
