...
use agent_companion_core::server::{ConnectionHandle, Handler};
...
pub struct Bridge {
    pub path: String,
    /// Whether an Agent Companion that was already running was stood down to get here.
    pub replaced: bool,
...
/// held by another Agent Companion is not that: see [`crate::single`].
pub fn start() -> io::Result<Bridge> {
    let (events_tx, events_rx) = channel();
    let (ready_tx, ready_rx) = channel();

    std::thread::Builder::new()
        .name("agent-companion-pipe".to_string())
        .spawn(move || {
...
impl Handler for Forwarder {
    fn on_envelope(&self, envelope: Envelope, connection: ConnectionHandle) {
        ...
        // Which events Agent Companion actually holds open, decided here rather than on
