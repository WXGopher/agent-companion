//! Stable Windows Terminal tab/pane identities captured at an Agent Companion launch.
...
pub const ENV: &str = "AGENT_COMPANION_TERMINAL_TARGET";
...
impl Target {
    pub fn with_env(
    ...
    ) -> agent_companion_core::protocol::TerminalMeta {
        ...
        agent_companion_core::protocol::TerminalMeta {
            ...
            ancestors: vec![agent_companion_core::protocol::ProcessRef {
                pid: self.pid,
...
pub fn from_meta(meta: &agent_companion_core::protocol::TerminalMeta) -> Option<Target> {
    serde_json::from_str(
        meta.env
            .get(ENV)
            .or_else(|| meta.env.get("ATOLL_TERMINAL_TARGET"))?
            .as_str()?,
    )
    .ok()
}
...
mod tests {
    ...
    fn native_hidden_tab_and_running_split_return_to_captured_pane() {
        ...
        let name = agent_companion_core::compat::var("AGENT_COMPANION_NATIVE_TEST_WINDOW")
            .expect("dedicated Terminal window name");
        let result_file = agent_companion_core::compat::var("AGENT_COMPANION_NATIVE_TEST_RESULT")
            .expect("test result path");
        let marker = format!("agent-companion-pane-test-{}", std::process::id());
        println!("{marker}");
            ...
            assert!(
                std::process::Command::new("wt.exe")
                    .args([
                        ...
                        "cmd.exe",
                        "/k",
                        "title Agent Companion test peer"
                    ])
                    .status()
                    .unwrap()
                    .success()
