...
mod tests {
    ...
    fn native_plain_cli_returns_to_its_own_terminal() {
        let home = agent_companion_core::install::codex_home().unwrap();
        let id = agent_companion_core::compat::var("AGENT_COMPANION_CLI_TEST_SESSION")
            .expect("live CLI session id");
        let expected: u32 = agent_companion_core::compat::var("AGENT_COMPANION_CLI_TEST_PID")
            .unwrap()
            .parse()
            .unwrap();
        let sessions = agent_companion_core::codex::SessionCache::default()
