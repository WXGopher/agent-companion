...
@MainActor enum PageSwitchTests {
    ...
    static func run(output: URL) throws {
        for camera in [false, true] {
            for availableHeight: CGFloat in [956, 280] {
                for (count, longError) in [(8, true), (0, false), (8, false)] {
                    try verify(camera: camera, availableHeight: availableHeight, count: count, longError: longError, output: output)
                }
            }
        }
        print("Page switching: stable panel, summary/footer pixels, persistent scroll views and offsets, loading/error/cache transitions passed")
    }

    private static func verify(camera: Bool, availableHeight: CGFloat, count: Int, longError: Bool, output: URL) throws {
        ...
        // Use the real deferred @Published sizing path with motion enabled.
        let subscription = model.objectWillChange.sink {
            DispatchQueue.main.async { presentation.update(screen: screen, availableHeight: availableHeight) }
        }
        defer { subscription.cancel(); presentation.stop(); model.stop(); panel.close() }
        presentation.update(screen: screen, availableHeight: availableHeight, animated: false)
        ...
        for step in 0..<12 {
            ...
            for tick in 0..<12 {
                ...
                precondition(surface.subviews[0] === host && scrollViews(in: surface).elementsEqual(scrolls, by: { $0 === $1 }),
                             "Switching Tasks / Usage recreated the host or a scroll view")
                let pixels = capture(surface)
                verifyAnchors(pixels, baseline: baseline, frame: frame, compactHeight: model.compactHeight)
                verifyContent(pixels, frame: frame, compactHeight: model.compactHeight)
            ...
            if step == 0 || step == 1 {
                try capture(surface).representation(using: .png, properties: [:])!.write(to:
                    output.appendingPathComponent("pages-\(camera)-\(Int(availableHeight))-\(count)-\(longError)-\(step).png"))
            }
        }
        ...
        for usage in [true, false, true, false] {
            ...
            precondition(scrolls.map { $0.contentView.bounds.origin } == offsets,
                         "Page switching lost a saved scroll position: \(offsets) → \(scrolls.map { $0.contentView.bounds.origin }), sizes \(sizes) → \(scrolls.map { "viewport \($0.contentSize), document \(String(describing: $0.documentView?.bounds.size))" }), camera \(camera), height \(availableHeight), count \(count), error \(longError), usage \(usage)")
        }
    }
    ...
    private static func verifyAnchors(_ pixels: NSBitmapImageRep, baseline: NSBitmapImageRep,
                                      frame: NSRect, compactHeight: CGFloat) {
        ...
        for range in [4..<Int((compactHeight - 4) * scale),
            ...
            for y in range where y % 3 == 0 {
                for x in stride(from: 20, to: pixels.pixelsWide - 20, by: 3) {
                    ...
                    for delta in [before.redComponent - after.redComponent, before.greenComponent - after.greenComponent,
                        ...
                        precondition(abs(delta) <= 2.0 / 255, "The summary or footer flashed during a page switch")
                    }
                }
            }
        }
    ...
    private static func verifyContent(_ pixels: NSBitmapImageRep, frame: NSRect, compactHeight: CGFloat) {
        let scale = CGFloat(pixels.pixelsHigh) / frame.height
        var ink = 0
        for y in stride(from: Int((compactHeight + 65) * scale), to: Int((frame.height - 80) * scale), by: 2) {
