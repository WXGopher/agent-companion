...
//! behavior a user gets without Agent Companion installed. There is deliberately no error
...
#[cfg(not(windows))]
mod ancestry {
    pub fn collect() -> Vec<agent_companion_core::protocol::ProcessRef> {
        Vec::new()
    }
}
