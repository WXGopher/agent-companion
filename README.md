...
**下载 / Download v0.3.11:** [macOS Apple Silicon](https://github.com/WXGopher/agent-companion/releases/download/v0.3.11/agent-companion-v0.3.11-macos-arm64.zip) · [Windows x86_64](https://github.com/WXGopher/agent-companion/releases/download/v0.3.11/agent-companion-v0.3.11-windows-x86_64.zip) · [发布说明 / Release notes](https://github.com/WXGopher/agent-companion/releases/tag/v0.3.11)
...
| 平台 | 本版功能 | 启动方式 |
| --- | --- | --- |
| Windows x86_64 | 任务栏、Tasks / Usage、订阅 Token 统计、分区设置、审批、通知及启动器 | `agent-companion.exe`；独立编辑器用 `agent-companion.exe codex-tui` |
...
**Usage** 页面显示 Codex 订阅的额度窗口、重置时间、累计 Token、单日峰值、最近七个有记录日期的用量、连续使用天数及最长任务时间。顶部固定显示 **Tasks / Usage**，随时一键切回任务；两页共用稳定的内容区域，切换不改变窗口大小，并各自保留滚动位置。也可点击周额度卡片进入用量。展开字体和间距缩小，大号 Token 数字调至约 13.5 pt。复用 Codex 的 ChatGPT 登录，无需 API Key。仅在用量页按需读取，五分钟内切换页面复用缓存，页面打开期间每五分钟自动刷新，也可手动刷新。查询不启动模型推理，不产生推理 Token 消耗。没有数据时显示 `—`；Token 总数与订阅剩余额度是不同指标，服务端数据可能延迟，也不提供账号级输入／缓存／输出拆分。详见[订阅用量说明](docs/MACOS_NOTCH.md#订阅用量v0310)。
...
- **Tasks / Usage**：固定导航切换任务与 Codex 订阅用量，两页各自保留滚动位置。Tasks 支持 Active / Finished；Usage 显示额度窗口与重置时间、累计／峰值 Token、最近七个有记录日期、连续使用天数及最长任务。复用 Codex CLI 的 ChatGPT 登录，无需 API Key；五分钟缓存、自动／手动刷新，离开页面取消读取。缺失指标显示 `—`，Token 总数不等于剩余额度。
...
- **设置与托盘**：分为 Codex CLI、Taskbar & app 和 Integrations；状态栏使用与 macOS 共享的实时预览和组件列表，切页保留草稿。支持开机启动、通知、代理显隐、颜色阈值及 hooks 管理。应用偏好自动保存，Codex 状态栏通过 Apply changes 保存。
- **Codex CLI 状态栏定制**：Settings → Codex CLI，勾选组件实时预览，Apply 保存，Restore Codex defaults 恢复默认；支持全部隐藏。
...
v0.3.11 将 Windows 设置与 Tasks / Usage 对齐 macOS。Windows 本地验证覆盖构建、Clippy、单元及集成测试、真实订阅读取，以及浅色／深色、100%／150%／200% 缩放下的界面渲染和交互。部分截图来自较早版本。macOS 保留既有功能，本次共享设置组件由跨平台 CI 验证；物理显示器拔插、全屏和 Terminal.app 跳转仍未覆盖，详见[验证范围](docs/MACOS_NOTCH.md)。
...
macOS 可从展开刘海后的齿轮 **Settings（设置）** 定制 TUI 状态栏；Windows 可在设置的 **Codex CLI** 页打开。两平台也可运行 `agent-companion codex-tui`。勾选表示显示，取消勾选表示隐藏；预览使用示例数据，不会发起模型请求。组件与默认预览按本机 Codex CLI **0.154.0** 的 `/statusline` 核对，包含模型、推理强度、目录、Git、上下文、额度、token 和会话信息。实际终端会省略无数据的组件，并按终端宽度显示。
...
```powershell
gh attestation verify agent-companion-v0.3.11-windows-x86_64.zip --repo WXGopher/agent-companion --source-ref refs/tags/v0.3.11 --deny-self-hosted-runners
```
...
- **Tasks / Usage**: fixed navigation with independent scroll positions. Tasks supports Active / Finished; Usage shows Codex subscription limits and resets, lifetime/peak tokens, the last seven reported days, streaks and longest turn. It reuses the Codex CLI ChatGPT login with a five-minute cache, automatic/manual refresh and cancellation on leaving. Missing metrics show `—`; token totals do not measure remaining allowance.
...
- **Settings and tray**: separate Codex CLI, Taskbar & app and Integrations sections. The status bar shares the macOS preview and component list, preserving drafts across pages. App preferences save automatically; Codex status-bar changes use Apply changes.
- **Codex CLI status bar**: open Settings → Codex CLI, toggle components in a live preview, then Apply or Restore Codex defaults. All components can be hidden.
...
v0.3.11 brings the macOS settings layout and Tasks / Usage experience to Windows. Local Windows checks cover builds, Clippy, unit/integration tests, a live subscription read, and light/dark rendering and interaction at 100%/150%/200% scaling. Some screenshots show earlier versions. Existing macOS features are retained; shared settings components are checked in cross-platform CI. Physical display hot-plugging, fullscreen and Terminal.app navigation remain outside the completed [verification scope](docs/MACOS_NOTCH.md).
