...
- 放大 macOS 菜单栏圆点、增强专用配色与描边，并将运行呼吸最低不透明度提高到 80%。
- 用合成数据检查浅色、深色、蓝色背景及呼吸暗相位；状态含义、额度刷新和减少动态效果行为继续保留。[发布说明](releases/v0.3.20.md)

## v0.3.19：旧版 Dodex 启动修复与入口整理

- 增加桌面校验和已审计旧启动器修复，TUI 进程不再误阻止桌面启动。
- 兼容 Finder 自定义图标，继续验证官方签名与资源完整性。
- 源码仓库提供显式执行的 CLI 隔离、入口整理及 Dock 清理工具，不自动修改既有环境。[发布说明](releases/v0.3.19.md)

## v0.3.18：macOS 菜单栏、统一状态颜色与手动同步

- 移除刘海、悬停展开、显示器选择和独立 Dock 入口；菜单栏始终可见，重新打开应用显示任务／用量弹窗。
- Codex／Dodex 各自显示状态圆点：运行蓝色、等待／失败黄色、全部完成绿色、空闲或停止／未知灰色，与 Windows 任务栏一致；缓存读数如 `69%*` 保留到有效更新后去掉星号。
- 弹窗提供完整应用名、构建版本、运行数和持久化亮暗主题，保留页面、实例与滚动状态。
- 双开页显示双方实际路径，支持手动双向覆盖 `config.toml` 与全局 `AGENTS.md`，自动备份目标并保护未保存草稿。内嵌配置密钥会复制，`auth.json` 不会复制，目标的登录存储、数据库与日志设置保持独立。
- 原生弹窗／菜单栏、隔离设置窗口及核心同步回归已通过；实机验收边界见 [macOS 菜单栏](MACOS_MENU_BAR.md)，完整变化见[发布说明](releases/v0.3.18.md)。下方条目保留历史版本记录。

## v0.3.10：订阅用量、稳定列表切换与设置分区

- macOS 设置按 Codex CLI 与 Notch & app 分区，状态栏预览移至组件列表上方的横向长条；切换分区保留草稿。
- 状态栏组件使用单行名称，悬停显示详细说明。
- 刘海内新增订阅用量页：复用 Codex 登录读取账号 Token 活动与额度，不需要 API Key；按需读取、可手动刷新，展开宽度保持不变。
- Tasks / Usage 固定在顶部，滚动时也能直接返回；收紧展开字体与间距，五分钟内切换页面复用用量缓存。
- macOS 菜单栏额度改为每两分钟读取本地记录；启动时立即读取，任务与订阅用量刷新间隔不变。
- Active / Finished 共用固定视口和滚动视图，切换时不再伸缩面板或闪烁。
- 修复 Tasks / Usage 切换闪烁；两页共用稳定区域，独立保留滚动位置，隐藏页不进入辅助功能树。
- 新增空列表、长列表、长错误提示、矮窗口、逐帧像素和滚动位置回归。[发布说明](releases/v0.3.10.md)。
